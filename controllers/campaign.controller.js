...
export const createCampaign = async (req, res) => {
  try {
    ...
    const campaign = await Campaign.create({
      ...
      status: isPublishedBool ? (isScheduledBool ? 'scheduled' : 'draft') : 'draft',
      stats: {
  ...
  try {
    ...
    const [totalCount, campaigns, campaignStatsAgg] = await Promise.all([
      ...
      Campaign.find(matchFilter)
        .select(
          'name description recipient_type is_published is_scheduled scheduled_at sent_at stats status completion_duration_seconds template_id created_at platform'
        )
        .populate({
          path: 'template_id',
    ...
    return res.json({
      ...
      data: {
        ...
        recipients_pagination: excludeRecipients ? null : {
          currentPage: page,
          totalPages: Math.ceil((campaign.stats?.total_recipients || 0) / limit),
          totalItems: campaign.stats?.total_recipients || 0,
          itemsPerPage: limit
        }
      }
    });
...
export const updateCampaign = async (req, res) => {
  try {
    ...
    updateData.status = newIsPublished ? (newIsScheduled ? 'scheduled' : 'draft') : 'draft';
